...
void ChessBoard::reset_to_start_position() {
    ...
    DEBUG_ONLY(debug_validate_bitboards());
}
...
void ChessBoard::set_from_fen(const std::string& fen) {
    ...
    for (char c : board_part) {
        if (c == '/') {
            ...
            switch (c) {
                case 'P':
                    ChessBitboardUtils::set_bit(white_pawns, square_idx);
                    break;
                case 'N':
                    ChessBitboardUtils::set_bit(white_knights, square_idx);
                    break;
                case 'B':
                    ChessBitboardUtils::set_bit(white_bishops, square_idx);
                    break;
                case 'R':
                    ChessBitboardUtils::set_bit(white_rooks, square_idx);
                    break;
                case 'Q':
                    ChessBitboardUtils::set_bit(white_queens, square_idx);
                    break;
                case 'K':
                    ChessBitboardUtils::set_bit(white_king, square_idx);
                    break;
                case 'p':
                    ChessBitboardUtils::set_bit(black_pawns, square_idx);
                    break;
                case 'n':
                    ChessBitboardUtils::set_bit(black_knights, square_idx);
                    break;
                case 'b':
                    ChessBitboardUtils::set_bit(black_bishops, square_idx);
                    break;
                case 'r':
                    ChessBitboardUtils::set_bit(black_rooks, square_idx);
                    break;
                case 'q':
                    ChessBitboardUtils::set_bit(black_queens, square_idx);
                    break;
                case 'k':
                    ChessBitboardUtils::set_bit(black_king, square_idx);
                    break;
                default:
                    return;
            }
...
std::string ChessBoard::to_fen() const {
    ...
    for (int rank = 7; rank >= 0; --rank) {
        ...
        for (int file = 0; file < 8; ++file) {
            ...
            char piece_char = ' ';
            if (ChessBitboardUtils::test_bit(white_pawns, square_idx))
                piece_char = 'P';
            else if (ChessBitboardUtils::test_bit(white_knights, square_idx))
                piece_char = 'N';
            else if (ChessBitboardUtils::test_bit(white_bishops, square_idx))
                piece_char = 'B';
            else if (ChessBitboardUtils::test_bit(white_rooks, square_idx))
                piece_char = 'R';
            else if (ChessBitboardUtils::test_bit(white_queens, square_idx))
                piece_char = 'Q';
            else if (ChessBitboardUtils::test_bit(white_king, square_idx))
                piece_char = 'K';
            else if (ChessBitboardUtils::test_bit(black_pawns, square_idx))
                piece_char = 'p';
            else if (ChessBitboardUtils::test_bit(black_knights, square_idx))
                piece_char = 'n';
            else if (ChessBitboardUtils::test_bit(black_bishops, square_idx))
                piece_char = 'b';
            else if (ChessBitboardUtils::test_bit(black_rooks, square_idx))
                piece_char = 'r';
            else if (ChessBitboardUtils::test_bit(black_queens, square_idx))
                piece_char = 'q';
            else if (ChessBitboardUtils::test_bit(black_king, square_idx))
                piece_char = 'k';
...
void ChessBoard::apply_move(const Move& move, StateInfo& state_info) {
    ...
    if (active_player == PlayerColor::White) {
        switch (move.piece_moved_type_idx) {
            case PieceTypeIndex::PAWN:
                moving_piece_bb_ptr = &white_pawns;
                break;
            case PieceTypeIndex::KNIGHT:
                moving_piece_bb_ptr = &white_knights;
                break;
            case PieceTypeIndex::BISHOP:
                moving_piece_bb_ptr = &white_bishops;
                break;
            case PieceTypeIndex::ROOK:
                moving_piece_bb_ptr = &white_rooks;
                break;
            case PieceTypeIndex::QUEEN:
                moving_piece_bb_ptr = &white_queens;
                break;
            case PieceTypeIndex::KING:
                moving_piece_bb_ptr = &white_king;
                break;
            default:
                return;
        }
        ...
        switch (move.piece_moved_type_idx) {
            case PieceTypeIndex::PAWN:
                moving_piece_bb_ptr = &black_pawns;
                break;
            case PieceTypeIndex::KNIGHT:
                moving_piece_bb_ptr = &black_knights;
                break;
            case PieceTypeIndex::BISHOP:
                moving_piece_bb_ptr = &black_bishops;
                break;
            case PieceTypeIndex::ROOK:
                moving_piece_bb_ptr = &black_rooks;
                break;
            case PieceTypeIndex::QUEEN:
                moving_piece_bb_ptr = &black_queens;
                break;
            case PieceTypeIndex::KING:
                moving_piece_bb_ptr = &black_king;
                break;
            default:
                return;
        }
    ...
    if (move.piece_captured_type_idx != PieceTypeIndex::NONE) {
        ...
        if (state_info.captured_piece_color == PlayerColor::White) {
            switch (move.piece_captured_type_idx) {
                case PieceTypeIndex::PAWN:
                    captured_piece_bb_ptr = &white_pawns;
                    break;
                case PieceTypeIndex::KNIGHT:
                    captured_piece_bb_ptr = &white_knights;
                    break;
                case PieceTypeIndex::BISHOP:
                    captured_piece_bb_ptr = &white_bishops;
                    break;
                case PieceTypeIndex::ROOK:
                    captured_piece_bb_ptr = &white_rooks;
                    break;
                case PieceTypeIndex::QUEEN:
                    captured_piece_bb_ptr = &white_queens;
                    break;
                default:
                    return;
            }
            ...
            switch (move.piece_captured_type_idx) {
                case PieceTypeIndex::PAWN:
                    captured_piece_bb_ptr = &black_pawns;
                    break;
                case PieceTypeIndex::KNIGHT:
                    captured_piece_bb_ptr = &black_knights;
                    break;
                case PieceTypeIndex::BISHOP:
                    captured_piece_bb_ptr = &black_bishops;
                    break;
                case PieceTypeIndex::ROOK:
                    captured_piece_bb_ptr = &black_rooks;
                    break;
                case PieceTypeIndex::QUEEN:
                    captured_piece_bb_ptr = &black_queens;
                    break;
                default:
                    return;
            }
    ...
    if (move.is_promotion) {
        ...
        if (active_player == PlayerColor::White) {
            switch (move.promotion_piece_type_idx) {
                case PieceTypeIndex::KNIGHT:
                    promoted_bb_ptr = &white_knights;
                    break;
                case PieceTypeIndex::BISHOP:
                    promoted_bb_ptr = &white_bishops;
                    break;
                case PieceTypeIndex::ROOK:
                    promoted_bb_ptr = &white_rooks;
                    break;
                case PieceTypeIndex::QUEEN:
                    promoted_bb_ptr = &white_queens;
                    break;
                default:
                    return;
            }
            ...
            switch (move.promotion_piece_type_idx) {
                case PieceTypeIndex::KNIGHT:
                    promoted_bb_ptr = &black_knights;
                    break;
                case PieceTypeIndex::BISHOP:
                    promoted_bb_ptr = &black_bishops;
                    break;
                case PieceTypeIndex::ROOK:
                    promoted_bb_ptr = &black_rooks;
                    break;
                case PieceTypeIndex::QUEEN:
                    promoted_bb_ptr = &black_queens;
                    break;
                default:
                    return;
            }
...
#ifdef DEBUG
bool ChessBoard::debug_validate_bitboards() const {
    return debug_validate_bitboards(std::cerr);
}

bool ChessBoard::debug_validate_bitboards(std::ostream& out) const {
    const std::array<NamedBitboard, 12> pieces = {{
    ...
    for (const NamedBitboard& piece : pieces) {
        const uint64_t overlap = seen & piece.value;
        if (overlap != 0ULL) {
            out << "DEBUG: Bitboard overlap in " << piece.name
                << ": 0x" << std::hex << overlap << std::dec << '\n';
            valid = false;
        }
        seen |= piece.value;
    }

    const uint64_t expected_white =
        white_pawns | white_knights | white_bishops | white_rooks | white_queens | white_king;
    const uint64_t expected_black =
        black_pawns | black_knights | black_bishops | black_rooks | black_queens | black_king;
    const uint64_t expected_occupied = expected_white | expected_black;

    if ((expected_white & expected_black) != 0ULL) {
        out << "DEBUG: White and black occupancy overlap: 0x"
            << std::hex << (expected_white & expected_black) << std::dec << '\n';
        valid = false;
    }

    if (white_occupied_squares != expected_white) {
        out << "DEBUG: white_occupied_squares mismatch. expected=0x"
            << std::hex << expected_white << " actual=0x" << white_occupied_squares << std::dec << '\n';
        valid = false;
    }

    if (black_occupied_squares != expected_black) {
        out << "DEBUG: black_occupied_squares mismatch. expected=0x"
            << std::hex << expected_black << " actual=0x" << black_occupied_squares << std::dec << '\n';
        valid = false;
    }

    if (occupied_squares != expected_occupied) {
        out << "DEBUG: occupied_squares mismatch. expected=0x"
            << std::hex << expected_occupied << " actual=0x" << occupied_squares << std::dec << '\n';
        valid = false;
    }

    if (ChessBitboardUtils::count_set_bits(white_king) != 1) {
        out << "DEBUG: Expected exactly one white king, found "
            << static_cast<int>(ChessBitboardUtils::count_set_bits(white_king)) << '\n';
        valid = false;
    }

    if (ChessBitboardUtils::count_set_bits(black_king) != 1) {
        out << "DEBUG: Expected exactly one black king, found "
            << static_cast<int>(ChessBitboardUtils::count_set_bits(black_king)) << '\n';
        valid = false;
    }

    if (zobrist_hash != calculate_zobrist_hash_from_scratch()) {
        out << "DEBUG: Zobrist hash mismatch. expected=0x"
            << std::hex << calculate_zobrist_hash_from_scratch()
            << " actual=0x" << zobrist_hash << std::dec << '\n';
        valid = false;
    }

    if (!valid) {
        out << "DEBUG: Invalid board FEN: " << to_fen() << '\n';
        debug_print_bitboards(out);
    }

    return valid;
}

void ChessBoard::debug_print_bitboards(std::ostream& out) const {
    out << "DEBUG: Board bitboards, FEN: " << to_fen() << '\n';

    for (int rank = 7; rank >= 0; --rank) {
        out << "DEBUG: " << (rank + 1) << " ";
        for (int file = 0; file < 8; ++file) {
            const int square_idx = ChessBitboardUtils::rank_file_to_square(rank, file);
            char piece = '.';
    ...
    out << "DEBUG:   a b c d e f g h\n";
    out << "DEBUG: white=0x" << std::hex << white_occupied_squares
        << " black=0x" << black_occupied_squares
        << " occupied=0x" << occupied_squares << std::dec << '\n';
}
#endif
...
int ChessBoard::get_piece_square_index(PieceTypeIndex piece_type_idx, PlayerColor piece_color) const {
    ...
    if (piece_color == PlayerColor::White) {
        switch (piece_type_idx) {
            case PieceTypeIndex::PAWN:
                target_bb = white_pawns;
                break;
            case PieceTypeIndex::KNIGHT:
                target_bb = white_knights;
                break;
            case PieceTypeIndex::BISHOP:
                target_bb = white_bishops;
                break;
            case PieceTypeIndex::ROOK:
                target_bb = white_rooks;
                break;
            case PieceTypeIndex::QUEEN:
                target_bb = white_queens;
                break;
            case PieceTypeIndex::KING:
                target_bb = white_king;
                break;
        ...
        switch (piece_type_idx) {
            case PieceTypeIndex::PAWN:
                target_bb = black_pawns;
                break;
            case PieceTypeIndex::KNIGHT:
                target_bb = black_knights;
                break;
            case PieceTypeIndex::BISHOP:
                target_bb = black_bishops;
                break;
            case PieceTypeIndex::ROOK:
                target_bb = black_rooks;
                break;
            case PieceTypeIndex::QUEEN:
                target_bb = black_queens;
                break;
            case PieceTypeIndex::KING:
                target_bb = black_king;
                break;
            case PieceTypeIndex::NONE:
                return 64;
        }
...
uint64_t ChessBoard::calculate_zobrist_hash_from_scratch() const {
    ...
    for (int square_idx = 0; square_idx < 64; ++square_idx) {
        if (ChessBitboardUtils::test_bit(white_pawns, square_idx))
            hash ^= zobrist_piece_keys[static_cast<int>(PieceTypeIndex::PAWN) + 0][square_idx];
        else if (ChessBitboardUtils::test_bit(white_knights, square_idx))
            hash ^= zobrist_piece_keys[static_cast<int>(PieceTypeIndex::KNIGHT) + 0][square_idx];
        else if (ChessBitboardUtils::test_bit(white_bishops, square_idx))
            hash ^= zobrist_piece_keys[static_cast<int>(PieceTypeIndex::BISHOP) + 0][square_idx];
        else if (ChessBitboardUtils::test_bit(white_rooks, square_idx))
            hash ^= zobrist_piece_keys[static_cast<int>(PieceTypeIndex::ROOK) + 0][square_idx];
        else if (ChessBitboardUtils::test_bit(white_queens, square_idx))
            hash ^= zobrist_piece_keys[static_cast<int>(PieceTypeIndex::QUEEN) + 0][square_idx];
        else if (ChessBitboardUtils::test_bit(white_king, square_idx))
            hash ^= zobrist_piece_keys[static_cast<int>(PieceTypeIndex::KING) + 0][square_idx];
        else if (ChessBitboardUtils::test_bit(black_pawns, square_idx))
            hash ^= zobrist_piece_keys[static_cast<int>(PieceTypeIndex::PAWN) + 6][square_idx];
        else if (ChessBitboardUtils::test_bit(black_knights, square_idx))
            hash ^= zobrist_piece_keys[static_cast<int>(PieceTypeIndex::KNIGHT) + 6][square_idx];
        else if (ChessBitboardUtils::test_bit(black_bishops, square_idx))
            hash ^= zobrist_piece_keys[static_cast<int>(PieceTypeIndex::BISHOP) + 6][square_idx];
        else if (ChessBitboardUtils::test_bit(black_rooks, square_idx))
            hash ^= zobrist_piece_keys[static_cast<int>(PieceTypeIndex::ROOK) + 6][square_idx];
        else if (ChessBitboardUtils::test_bit(black_queens, square_idx))
            hash ^= zobrist_piece_keys[static_cast<int>(PieceTypeIndex::QUEEN) + 6][square_idx];
        else if (ChessBitboardUtils::test_bit(black_king, square_idx))
            hash ^= zobrist_piece_keys[static_cast<int>(PieceTypeIndex::KING) + 6][square_idx];
    }
