...
void handlePositionCommand(const std::string &command_line)
{
	...
	{
		DEBUG_LOG("DEBUG: Invalid position command: " << sub_command);
		return;
	}
	...
	if (current_token_after_board_setup == "moves")
	{
		...
		while (ss >> move_str)
		{
			...
			{
				DEBUG_LOG("DEBUG: Invalid move encountered in 'position moves': " << move_str);
				DEBUG_LOG("DEBUG: Current FEN: " << board.to_fen());
				break;
			}
		}
	}
