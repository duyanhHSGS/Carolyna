...
int ChessAI::alphaBeta(ChessBoard& board, int depth, int alpha, int beta) {
    ...
    for (const auto& move : legal_moves) {
        ...
        if (entry.best_move.piece_moved_type_idx != PieceTypeIndex::NONE &&
            ...
            move_score = PIECE_SORT_VALUES[static_cast<int>(move.piece_captured_type_idx)] * 10 - PIECE_SORT_VALUES[static_cast<int>(move.piece_moved_type_idx)];
            move_score += 10000;
...
Move ChessAI::findBestMove(ChessBoard& board) {
    ...
    if (legal_moves.empty()) {
        DEBUG_LOG("DEBUG: Carolyna: No legal moves found. Game is likely over (checkmate or stalemate).");
    ...
    if (duration_ms > 0) {
        nodes_per_second =
            (static_cast<long long>(nodes_evaluated_count) * 1000) /
            duration_ms;
    } else if (nodes_evaluated_count > 0) {
        nodes_per_second =
            static_cast<long long>(nodes_evaluated_count) * 1000000;
    }

    DEBUG_LOG("DEBUG: Carolyna: Completed search to depth "
              << current_search_depth_set
    ...
    if (std::abs(final_display_score) >= ChessAI::MATE_VALUE) {
        score_string =
            ...
            std::to_string(
                (ChessAI::MATE_VALUE -
                 std::abs(final_display_score) +
                 current_search_depth_set) *
                (final_display_score > 0 ? 1 : -1));
    ...
    DEBUG_LOG("DEBUG: Carolyna : Chose move for "
              << (original_active_player == PlayerColor::White
                      ? "White"
                      : "Black")
              << ": "
